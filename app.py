...
app = Flask(__name__)
app.secret_key = "hc_decryptor_secure_2026"

# EXACT LOGINS YOU WANT
VALID_USERS = {
    "Cxlvin777": "Cxlvin777",
    "root": "admin",
    "guest": "1234"
...
# ==============================================
# YOUR FULL DECRYPTION CODE — NOTHING CHANGED
# ==============================================
class HCConstants:
    ...
    TOKEN_MAP: Dict[int, str] = {
        0:"payload",1:"proxy",2:"lockAllConfig",3:"blockedByRoot",4:"expiryTime",5:"noteEnabled",6:"notes",7:"sshField",
        8:"mobileDataAndLockProvider",9:"unlockUserAndPass",10:"ovpnConfig",11:"ovpnUserAndPass",12:"sni",13:"unlockUserAndPass2",
        14:"unknown14",15:"blockedByHwid",16:"cloudconfig",17:"psiphon",18:"name",19:"blockArea",
        20:"connectionMode",21:"blockedByPassword",22:"unknown22",23:"extraSniffer",24:"psiphon2",25:"v2rayEnabled",
        26:"v2rayConfig",27:"version",28:"slowdnsEnabled",29:"slowdnsServer",30:"slowdnsPublickey",31:"dnsResolver"
    }
...
class HCDecryptor:
    ...
    def execute(cls, file_bytes: bytes) -> Optional[str]:
        ...
        with contextlib.suppress(Exception):
            ...
            if not isinstance(json_obj, dict): return None
            ...
            if not target_cipher or not split_delim: return None
            ...
            for i, token in enumerate(xy_dec.split(str(split_delim))):
                ...
                final_out = token
                if is_new_format:
                    final_out = cls._decrypt_field(token, bytes(dynamic_nonce))
                else:
                    if cls._is_hex(token):
                        final_out = cls._abc_decrypt(token, HCConstants.CHACHA_KEYS[7], bytes(dynamic_nonce))
...
def login():
    ...
    return jsonify({"ok": False, "msg": "Wrong username or password"}), 401

@app.route('/api/decrypt', methods=['POST'])
def decrypt():
    if 'file' not in request.files:
        return jsonify({"ok": False, "err": "No file selected"}), 400
    f = request.files['file']
    if not f.filename.lower().endswith('.hc'):
        return jsonify({"ok": False, "err": "Only .hc files allowed"}), 400
    res = run(f.read())
    if res:
        return jsonify({"ok": True, "data": res})
    return jsonify({"ok": False, "err": "Decryption failed"}), 500
