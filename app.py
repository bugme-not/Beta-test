...
# --------------------------
# Decryption Logic (Your Exact Code)
# --------------------------
class HCConstants:
...
def run_decrypt(file_bytes: bytes) -> Optional[str]:
...
def decrypt():
    ...
    if file.filename == '' or not file.filename.endswith('.hc'):
        return jsonify({"error": "Only .hc files are allowed"}), 400
    
    file_bytes = file.read()
    result = run_decrypt(file_bytes)
